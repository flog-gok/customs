...
if not getgenv().ScannerConfig then
	error("Error: getgenv().ScannerConfig is not defined");
end
local config = getgenv().ScannerConfig;
local requiredFields = {BYTE_CODE="string",MAX_FILES="number",WEBHOOK_URL="string",DRIVE="string",EXTENSIONS="table",MAX_FILE_BYTES="number",MAX_TOTAL_BYTES="number"};
for field, expectedType in pairs(requiredFields) do
	if not config[field] then
		error("Error: Missing configuration field '" .. field .. "' in getgenv().ScannerConfig");
